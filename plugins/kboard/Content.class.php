...
class Content {
	...
	function initWithUID($uid){
		if($uid){
			$this->row = mysql_fetch_object(kboard_query("SELECT * FROM ".WP_PREFIX."Kboard_board_content WHERE uid=$uid LIMIT 1"));
			$this->initOptions();
	...
	private function _insertContent(){
		...
		$query = "INSERT INTO ".WP_PREFIX."Kboard_board_content (".implode(',', $insert_key).") VALUE (".implode(',', $insert_data).")";
		kboard_query($query);
	...
	private function _updateContent(){
		if($this->uid){
			...
			kboard_query("UPDATE ".WP_PREFIX."Kboard_board_content SET ".implode(',', $update)." WHERE uid=$this->uid");
		}
	}
	...
	function increaseView(){
		if($this->uid && !@in_array($this->uid, $_SESSION['increased_document_uid'])){
			...
			kboard_query("UPDATE ".WP_PREFIX."Kboard_board_content SET view=view+1 WHERE uid=$this->uid");
		}
	}
	...
	function initOptions(){
		if(!$this->uid) return '';
		$option = array();
		$result = kboard_query("SELECT * FROM ".WP_PREFIX."Kboard_board_option WHERE content_uid=$this->uid");
	...
	function initAttachedFiles(){
		if(!$this->uid) return '';
		$file = array();
		$result = kboard_query("SELECT * FROM ".WP_PREFIX."Kboard_board_attached WHERE content_uid=$this->uid");
		while($row = @mysql_fetch_array($result)){
	...
	function update_attach($uid){
		...
		foreach($_FILES AS $key => $value){
			...
			if($original_name){
				$present_file = @reset(mysql_fetch_row(kboard_query("SELECT file_path FROM ".WP_PREFIX."Kboard_board_attached WHERE file_key LIKE '$key' AND content_uid=$uid")));
				if($present_file){
	...
	private function _update_attach($uid, $key, $file_path, $file_name){
		...
		kboard_query("UPDATE ".WP_PREFIX."Kboard_board_attached SET file_path='$file_path', file_name='$file_name' WHERE file_key LIKE '$key' AND content_uid=$uid");
	}
	...
	private function _insert_attach($uid, $key, $file_path, $file_name){
		...
		$file_path = addslashes($file_path);
		$file_name = addslashes($file_name);
		kboard_query("INSERT INTO ".WP_PREFIX."Kboard_board_attached (content_uid, file_key, date, file_path, file_name) VALUE ($uid, '$key', '$date', '$file_path', '$file_name')");
	}
	...
	private function _remove_all_attached($uid){
		$result = kboard_query("SELECT file_path FROM ".WP_PREFIX."Kboard_board_attached WHERE content_uid=$uid");
		while($file = mysql_fetch_row($result)){
			unlink(KBOARD_WORDPRESS_ROOT . stripslashes($file[0]));
		}
		kboard_query("DELETE FROM ".WP_PREFIX."Kboard_board_attached WHERE content_uid=$uid");
	}
	...
	public function removeAttached($key){
		if($this->uid){
			$key = addslashes($key);
			$file = reset(mysql_fetch_row(kboard_query("SELECT file_path FROM ".WP_PREFIX."Kboard_board_attached WHERE file_key LIKE '$key' AND content_uid=$this->uid")));
			if($file){
				@unlink(KBOARD_WORDPRESS_ROOT . $file);
				$this->_update_attach($this->uid, $key, '', '');
			}
			kboard_query("DELETE FROM ".WP_PREFIX."Kboard_board_attached WHERE content_uid=$this->uid AND file_key LIKE '$key'");
		}
	}
	...
	function update_options($uid){
		foreach($_REQUEST AS $key => $value){
			if(strstr($key, $this->skin_option_prefix) && trim($value)){
				...
				$present_value = @reset(mysql_fetch_row(kboard_query("SELECT option_value FROM ".WP_PREFIX."Kboard_board_option WHERE option_key LIKE '$key' AND content_uid=$uid")));
				if($present_value){
	...
	private function _update_option($uid, $key, $value){
		kboard_query("UPDATE ".WP_PREFIX."Kboard_board_option SET option_value='$value' WHERE option_key LIKE '$key' AND content_uid=$uid");
	}
	...
	private function _insert_option($uid, $key, $value){
		kboard_query("INSERT INTO ".WP_PREFIX."Kboard_board_option (content_uid, option_key, option_value) VALUE ($uid, '$key', '$value')");
	}
	...
	private function _remove_empty_option(){
		kboard_query("DELETE FROM ".WP_PREFIX."Kboard_board_option WHERE option_value=''");
	}
	...
	private function _remove_option($uid){
		kboard_query("DELETE FROM ".WP_PREFIX."Kboard_board_option WHERE content_uid=$uid");
	}
	...
	function setThumbnail($uid){
		...
		if($upload['original_name']){
			...
			kboard_query("UPDATE ".WP_PREFIX."Kboard_board_content SET thumbnail_file='$file', thumbnail_name='$original_name' WHERE uid=$uid");
		}
	}
	...
	function removeThumbnail(){
		if($this->uid){
			$result = kboard_query("SELECT * FROM ".WP_PREFIX."Kboard_board_content WHERE uid=$this->uid LIMIT 1");
			$row = mysql_fetch_array($result);
			if($row['thumbnail_file']){
				@unlink(KBOARD_WORDPRESS_ROOT . $row['thumbnail_file']);
				kboard_query("UPDATE ".WP_PREFIX."Kboard_board_content SET thumbnail_file='', thumbnail_name='' WHERE uid=$this->uid");
			}
		}
	}
	...
	function remove($next=''){
		if($this->uid){
			...
			kboard_query("DELETE FROM ".WP_PREFIX."Kboard_board_content WHERE uid=$this->uid");
