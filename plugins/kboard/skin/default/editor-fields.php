...
<?php elseif($field['field_type'] == 'content'):?>
	<div class="kboard-content">
		<?php if($this->board->use_editor):?>
			<?php wp_editor($content->content, 'kboard_content', array('media_buttons'=>$this->board->isAdmin(), 'editor_height'=>400))?>
		<?php else:?>
			<textarea name="kboard_content" id="kboard_content"><?php echo $content->content?></textarea>
		<?php endif?>
	</div>
<?php elseif($field['field_type'] == 'non_member'):?>
	<?php if($this->board->viewUsernameField()):?>
...
<?php elseif($field['field_type'] == 'captcha'):?>
	<?php if($this->board->useCAPTCHA() && !$content->uid):?>
...
<?php elseif($field['field_type'] == 'attach'):?>
	<?php if($this->board->meta->max_attached_count > 0):?>
		<!-- 첨부파일 시작 -->
		<?php for($attached_index=1; $attached_index<=$this->board->meta->max_attached_count; $attached_index++):?>
		<div class="kboard-attr-row">
			<label class="attr-name" for="kboard-input-file<?php echo $attached_index?>"><?php echo $field_name?><?php echo $attached_index?></label>
...
<?php elseif($field['field_type'] == 'category1'):?>
	<?php if(!$this->board->isTreeCategoryActive()):?>
		<?php if($this->board->initCategory1()):?>
			<div class="kboard-attr-row <?php echo $field['class']?> <?php echo $required?>">
				<label class="attr-name" for="<?php echo esc_attr($meta_key)?>"><?php echo $field_name?></label>
				<div class="attr-value">
					<select id="<?php echo esc_attr($meta_key)?>" name="category1" class="<?php echo $required?>">
						...
						<?php while($this->board->hasNextCategory()):?>
						<option value="<?php echo $this->board->currentCategory()?>"<?php if($content->category1 == $this->board->currentCategory()):?> selected<?php endif?>><?php echo $this->board->currentCategory()?></option>
						<?php endwhile?>
					</select>
					<?php if(isset($field['description']) && $field['description']):?><div class="description"><?php echo esc_html($field['description'])?></div><?php endif?>
				</div>
...
<?php elseif($field['field_type'] == 'category2'):?>
	<?php if(!$this->board->isTreeCategoryActive()):?>
		<?php if($this->board->initCategory2()):?>
			<div class="kboard-attr-row <?php echo $field['class']?> <?php echo $required?>">
				<label class="attr-name" for="<?php echo esc_attr($meta_key)?>"><?php echo $field_name?></label>
				<div class="attr-value">
					<select id="<?php echo esc_attr($meta_key)?>" name="category2" class="<?php echo $required?>">
						<option value=""><?php echo __('Category', 'kboard')?> <?php echo __('Select', 'kboard')?></option>
						<?php while($this->board->hasNextCategory()):?>
						<option value="<?php echo $this->board->currentCategory()?>"<?php if($content->category2 == $this->board->currentCategory()):?> selected<?php endif?>><?php echo $this->board->currentCategory()?></option>
						<?php endwhile?>
					</select>
					<?php if(isset($field['description']) && $field['description']):?><div class="description"><?php echo esc_html($field['description'])?></div><?php endif?>
				</div>
...
<div class="kboard-attr-row <?php echo $field['class']?>">
	<label class="attr-name" for="<?php echo esc_attr($meta_key)?>"><?php echo $field_name?></label>
	<div class="attr-value">
		<input type="text" id="<?php echo esc_attr($meta_key)?>" name="title" value="<?php echo $content->title?$content->title:esc_attr($default_value)?>" placeholder="<?php echo esc_attr($placeholder)?>">
		<?php if(isset($field['description']) && $field['description']):?><div class="description"><?php echo esc_html($field['description'])?></div><?php endif?>
	</div>
</div>
...
<div class="kboard-attr-row <?php echo $field['class']?>">
	...
	<div class="attr-value">
		...
		<?php endif?>
		<?php do_action('kboard_skin_editor_option', $content, $this->board, $this->board->boardBuilder)?>
		<?php if(isset($field['description']) && $field['description']):?><div class="description"><?php echo esc_html($field['description'])?></div><?php endif?>
	</div>
</div>
...
<?php if(isset($field['hidden']) && $field['hidden']):?>
	<input type="hidden" id="<?php echo esc_attr($meta_key)?>" class="<?php echo $required?>" name="<?php echo $this->getOptionFieldName(esc_attr($meta_key))?>" value="<?php echo $content->option->{esc_attr($meta_key)}?$content->option->{esc_attr($meta_key)}:esc_attr($default_value)?>">
<?php else:?>
<div class="kboard-attr-row <?php echo $field['class']?> <?php echo $required?>">
	<label class="attr-name" for="<?php echo esc_attr($meta_key)?>"><?php echo esc_html($field['field_name'])?esc_html($field['field_name']):esc_html($field['field_label'])?></label>
	<div class="attr-value">
		<input type="text" id="<?php echo esc_attr($meta_key)?>" class="<?php echo $required?>" name="<?php echo $this->getOptionFieldName(esc_attr($meta_key))?>" value="<?php echo $content->option->{esc_attr($meta_key)}?$content->option->{esc_attr($meta_key)}:esc_attr($default_value)?>" placeholder="<?php echo esc_attr($placeholder)?>">
		<?php if(isset($field['description']) && $field['description']):?><div class="description"><?php echo esc_html($field['description'])?></div><?php endif?>
	</div>
</div>
<?php endif?>
...
<div class="kboard-attr-row <?php echo $field['class']?> <?php echo $required?>">
	<label class="attr-name" for="<?php echo esc_attr($meta_key)?>"><?php echo $field_name?></label>
	<div class="attr-value">
		<select id="<?php echo esc_attr($meta_key)?>" name="<?php echo $this->getOptionFieldName(esc_attr($meta_key))?>"class="<?php echo $required?>">
			...
			<?php foreach($field['row'] as $option_key=>$option_value):?>
				<?php if(isset($option_value['label']) && $option_value['label']):?>
					<?php if($content->option->{$meta_key}):?>
						<option value="<?php echo esc_attr($option_value['label'])?>"<?php if($this->isSavedOption($content->option->{$meta_key}, $option_value['label'])):?> selected<?php endif?>><?php echo esc_attr($option_value['label'])?></option>
					<?php else:?>
						<option value="<?php echo esc_attr($option_value['label'])?>"<?php if($this->isSavedOption($default_value, $option_key)):?> selected<?php endif?>><?php echo esc_attr($option_value['label'])?></option>
					<?php endif?>
				<?php endif?>
			<?php endforeach?>
		</select>
...
<?php elseif($field['field_type'] == 'radio'):?>
	...
	<div class="kboard-attr-row <?php echo $field['class']?> <?php echo $required?>">
		...
		<?php foreach($field['row'] as $option_key=>$option_value):?>
			<?php if(isset($option_value['label']) && $option_value['label']):?>
				<?php if($content->option->{$meta_key}):?>
					<label><input type="radio" name="<?php echo $this->getOptionFieldName(esc_attr($meta_key))?>"class="<?php echo $required?>"<?php if($this->isSavedOption($content->option->{$meta_key}, $option_value['label'])):?> checked<?php endif?> value="<?php echo esc_attr($option_value['label'])?>"><?php echo esc_attr($option_value['label'])?></label>
				<?php else:?>
					<label><input type="radio" name="<?php echo $this->getOptionFieldName(esc_attr($meta_key))?>"class="<?php echo $required?>"<?php if($this->isSavedOption($default_value, $option_key)):?> checked<?php endif?> value="<?php echo esc_attr($option_value['label'])?>"><?php echo esc_attr($option_value['label'])?></label>
				<?php endif?>
			<?php endif?>
		<?php endforeach?>
		<?php if(isset($field['description']) && $field['description']):?><div class="description"><?php echo esc_html($field['description'])?></div><?php endif?>
		...
		<div class="attr-value">
			<?php foreach($field['row'] as $option_key=>$option_value):?>
				<?php if(isset($option_value['label']) && $option_value['label']):?>
					...
					<?php if($content->option->{$meta_key}):?>
						<label><input type="checkbox" name="<?php echo $this->getOptionFieldName(esc_attr($meta_key))?>[]"class="<?php echo $required?>"<?php if($this->isSavedOption($content->option->{$meta_key}, $option_value['label'])):?> checked<?php endif?> value="<?php echo esc_attr($option_value['label'])?>"><?php echo esc_attr($option_value['label'])?></label>
					<?php else:?>
						<label><input type="checkbox" name="<?php echo $this->getOptionFieldName(esc_attr($meta_key))?>[]"class="<?php echo $required?>"<?php if($this->isSavedOption($default_value, $option_value['label'])):?> checked<?php endif?> value="<?php echo esc_attr($option_value['label'])?>"><?php echo esc_attr($option_value['label'])?></label>
					<?php endif?>
				<?php endif?>
			<?php endforeach?>
			<?php if(isset($field['description']) && $field['description']):?><div class="description"><?php echo esc_html($field['description'])?></div><?php endif?>
...
<div class="kboard-attr-row <?php echo $field['class']?> <?php echo $required?>">
	<label class="attr-name" for="<?php echo esc_attr($meta_key)?>"><?php echo $field_name?></label>
	<div class="attr-value">
		<textarea id="<?php echo esc_attr($meta_key)?>" name="<?php echo $this->getOptionFieldName(esc_attr($meta_key))?>"class="<?php echo $required?>" placeholder="<?php echo esc_attr($placeholder)?>"><?php echo $content->option->{esc_attr($meta_key)}?$content->option->{esc_attr($meta_key)}:esc_attr($default_value)?></textarea>
		<?php if(isset($field['description']) && $field['description']):?><div class="description"><?php echo esc_html($field['description'])?></div><?php endif?>
	</div>
</div>
...
<div class="kboard-attr-row <?php echo $field['class']?> <?php echo $required?>">
	<label class="attr-name" for="<?php echo esc_attr($meta_key)?>"><?php echo $field_name?></label>
	<div class="attr-value">
		<?php wp_editor($content->option->{esc_attr($meta_key)}?$content->option->{esc_attr($meta_key)}:esc_attr($default_value), $this->getOptionFieldName(esc_attr($meta_key)), array('media_buttons'=>$this->board->isAdmin(), 'editor_height'=>400, 'editor_class'=>$required))?>
		<?php if(isset($field['description']) && $field['description']):?><div class="description"><?php echo esc_html($field['description'])?></div><?php endif?>
	</div>
</div>
<?php endif?>