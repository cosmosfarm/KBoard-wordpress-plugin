...
/*
 * 게시글 작성 페이지에 임시저장을 추가한다.
 */
add_action('kboard_skin_editor_option', 'kboard_skin_editor_option_temporary_save', 10, 3);
function kboard_skin_editor_option_temporary_save($content, $board, $builder){
	if($board->isAdmin()){
	?>
	<label class="attr-value-option attr-value-temporary-save">
		<input type="hidden" name="status" value="">
		<input type="checkbox" name="status" value="hide"<?php if($content->status == 'hide'):?> checked<?php endif?>> <?php echo __('Temporary save', 'kboard')?>
	</label>
	<?php
	}
}
/*
add_action('kboard_skin_field_after', 'kboard_skin_field_after_temporary_save', 10, 3);
function kboard_skin_field_after_temporary_save($field, $content, $board){
	if($field['field_type'] == 'title'){
		if($board->isAdmin()){
		?>
		<div class="kboard-attr-row kboard-attr-select kboard-attr-temporary-save">
			<label class="attr-name" for="kboard-status"><?php echo __('Temporary save', 'kboard')?></label>
			<div class="attr-value">
				<select id="kboard-status" name="status">
					<option value="">보임</option>
					<option value="hide"<?php if($content->status == 'hide'):?> selected<?php endif?>><?php echo __('Temporary save', 'kboard')?></option>
				</select>
			</div>
		</div>
		<?php
...
/*
 * 게시글 상태 목록에 임시저장을 추가한다.
 */
add_filter('kboard_content_status_list', 'kboard_content_status_list_temporary_save', 10, 1);
function kboard_content_status_list_temporary_save($status_list){
	$status_list['hide'] = __('Temporary save', 'kboard');
	return $status_list;
}

/*
 * 게시판 목록에서 보여줄 게시글 상태에 임시저장을 추가한다.
 */
add_filter('kboard_get_list_status', 'kboard_get_list_status_temporary_save', 10, 2);
function kboard_get_list_status_temporary_save($get_list_status, $board_id){
	if(is_admin()){
		$get_list_status = array('', 'pending_approval', 'hide');
	}
	else{
...
/*
 * 게시글 목록 페이지와 본문 페이지에서 임시저장 문구를 추가한다.
 */
add_filter('kboard_content_value', 'kboard_content_value_temporary_save', 10, 3);
function kboard_content_value_temporary_save($value, $name, $content){
	if(in_array(kboard_mod(), array('list', 'document')) && $name == 'title' && $content->status == 'hide'){
		$value = '[' . __('Temporary save', 'kboard') . '] ' .$value;
	}
	return $value;
}
