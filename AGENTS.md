...
3. [ ] **⭐ 중복 선택자 확인 ⭐**: 동일한 CSS 선택자가 파일 내 **여러 위치에 정의**되어 있지 않은가? (아래 디버깅 가이드 참고)
4. [ ] **아이콘 확인**: Lock, Reply 아이콘이 깨지지 않고 벡터로 잘 나오는가?
5. [ ] **PC vs Mobile 정책 검증 (반응형)**:
    - **PC View**:
...
6. [ ] **로직 검증**: 원본 스킨의 고유 기능(예: 아바타 출력, 썸네일 출력)이 누락되지 않았는가?

## 5-1. CSS 디버깅 가이드 (Debugging Guide)

스타일 문제 발생 시 반드시 따라야 할 디버깅 절차입니다.

### 🔍 중복 선택자 문제 (Duplicate Selector Issue)
**증상**: 특정 요소의 스타일이 의도대로 적용되지 않음. Default 스킨과 분명히 다르게 보임.

**원인**: CSS 파일 내에 **동일한 선택자가 여러 번 정의**되어 있을 때, 나중에 정의된 스타일이 앞의 스타일을 덮어씁니다.
*   예: `#kboard-avatar-editor .kboard-control`가 `957`라인과 `1505`라인 두 곳에 정의되어 있으면, 뒤쪽 정의가 적용되지만 **앞쪽 정의의 일부 속성이 남아 충돌**을 일으킬 수 있습니다.

**해결 절차**:
1.  문제가 되는 선택자를 **파일 전체에서 검색** (`grep` 또는 Ctrl+F):
    ```bash
    grep -n ".kboard-control" avatar/style.css
    ```
2.  동일 선택자가 **2번 이상** 나오면 → **중복 정의 확인**.
3.  의도하지 않은 중복은 **하나로 통합**하거나 **불필요한 블록 삭제**.

### ⚠️ 흔한 실수 사례
*   **스킨 복제 시**: `default` 스킨에서 복사한 스타일과, 기존 `[new-skin]`에 있던 커스텀 스타일이 **같은 선택자**를 사용하여 충돌.
*   **패치/수정 시**: 파일 끝에 새 스타일을 추가했지만, 파일 중간에 **같은 요소에 대한 기존 스타일**이 남아 있음.
